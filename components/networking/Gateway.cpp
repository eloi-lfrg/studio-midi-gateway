...
Gateway::Gateway(size_t queueDepth) : m_queue(nullptr), m_task(nullptr), m_onFrame(nullptr), m_queueDepth(queueDepth) {
  s_instance = this;
}
...
esp_err_t Gateway::begin() {
  ...
  BaseType_t created = xTaskCreate(s_processEntry, "networking_rx", 4096, this, 5, &m_task);
  if (created != pdPASS) {
...
void Gateway::processTask() {
  ...
  while (true) {
    if (xQueueReceive(m_queue, &frame, portMAX_DELAY) != pdTRUE)
      continue;
    ESP_LOGD(TAG, "rx from %02X:%02X:%02X:%02X:%02X:%02X len=%zu", frame.srcMac[0], frame.srcMac[1], frame.srcMac[2],
             frame.srcMac[3], frame.srcMac[4], frame.srcMac[5], frame.payloadLen);
    if (m_onFrame) {
      m_onFrame(frame);
    }
  }
